...
const useFilters = (args: IProps) => {
  ...
  useEffect(() => {
    const byTaskName = tasks.filter((task) =>
      task.name
        .toLowerCase()
        .includes(debouncedValue.toLowerCase()),
    );
    const byType = byTaskName.filter((task) =>
      type === 'All' ? task : task.type === type,
    );
    const byPlannedStart = byType.filter((task) => {
      ...
      const pickedISO = plannedStart
        .toISOString()
        .slice(0, 10);
      const plannedISO = new Date(task.plannedStart)
        .toISOString()
        .slice(0, 10);
    ...
    const byPlannedEnd = byPlannedStart.filter((task) => {
      ...
      const pickedISO = plannedEnd
        .toISOString()
        .slice(0, 10);
      const plannedISO = new Date(task.plannedEnd)
        .toISOString()
        .slice(0, 10);
    ...
    const byStarted = byPlannedEnd.filter((task) => {
      ...
      const plannedISO = new Date(task.startedTime)
        .toISOString()
        .slice(0, 10);
    ...
    const byEnded = byStarted.filter((task) => {
      ...
      const plannedISO = new Date(task.endedTime)
        .toISOString()
        .slice(0, 10);
    ...
    const byCompleted = byEnded.filter((task) =>
